...
def train():
    ...
    for step in range(conf.train.min_step + 1):
        ...
        with amp.autocast():
            ...
            imagenet_loss = torch.sum(imagenet_loss, dim=-1)
            #print(imagenet_loss.shape,out_s.shape,out_s,label_s.shape,label_s)
            out=nn.LogSoftmax(dim=-1)(out_s)
            #print(out,label_s)
            ce_loss=torch.zeros(out.size(0))
            ind=0
            for ii in label_s:
                ce_loss[ind]=-out[ind,ii]
                ind+=1
            ce_loss=ce_loss.to(device)
            total_loss=1.0*imagenet_loss+ce_loss
            #print(total_loss.shape)
            alpha=0.5
            k=int(alpha*total_loss.size(0))
            #print(k)
            th=torch.kthvalue(total_loss,k)[0]
            np_loss=total_loss.cpu().detach().numpy()
            np_loss1=np.where(np_loss<th.item(),np_loss,0.0)
            total_loss1=torch.from_numpy(np_loss1)
            #print(total_loss,total_loss1)
            loss_s = torch.mean(total_loss)
            #print(loss_s.item(),torch.mean(ce_loss).item(),torch.mean(imagenet_loss).item())
